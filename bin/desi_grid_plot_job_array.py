...
gp_resultsdir = projectdir                                                               
cf.makedirectory(gp_resultsdir) 
...
# Run Fisher Forecast                                                                                               
full_masses = np.geomspace(0.01, 10., 10)                                                    
full_temps = np.linspace(0.79, 1.5, 12)    
...
gp_forecastset = [[cf.forecast(                                               
    ...
    for fidrowidx, fidrowvals in enumerate(gp_fiducialset)] 
...
for frowidx, frowval in enumerate(gp_forecastset):                            
    for fidx, fcst in enumerate(frowval):                                       
        if type(fcst.fisher)==type(None):                                       
            ...
            dill.dump_session(os.path.join(gp_resultsdir, 'gp_'+str(temp_index)+'_'+str(mass_index)+'.db'))
        else:                                                                   
