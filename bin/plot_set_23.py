...
#ps23_resultsdir = os.path.join(projectdir, 'results', 'ps23')
ps23_resultsdir = projectdir
#ps23_convergencedir = os.path.join(ps23_resultsdir, 'convergence')
cf.makedirectory(ps23_resultsdir)
#cf.makedirectory(ps23_convergencedir) 
...
for frowidx, frowval in enumerate(ps23_forecastset): 
    for fidx, fcst in enumerate(frowval): 
        if type(fcst.fisher)==type(None):
            ...
            dill.dump_session(os.path.join(ps23_resultsdir, str(frowidx)+'_'+str(fidx)+'_ps23.db'))
        else: 
...
plt.semilogx(masses, data[0, :], label=r"$\sigma_{g_\chi}$, DESI, 0.91K", linestyle='solid')
plt.xlabel(r"$M_\chi$ [eV]", fontsize=24)
plt.ylabel(r"$\sigma_{g_\chi}$", fontsize=24)
plt.legend(fontsize=18, loc='lower left')
plt.tick_params(axis='x', which='minttor')
plt.grid(True, which='minor')
plt.savefig(ps23_resultsdir + "/plotA23a.png")

plt.figure(figsize=(15,7.5))
plt.loglog(masses, 1./data[0, :], label=r"$\sigma_{g_\chi}$, DESI, 0.91K", linestyle='solid')
plt.xlabel(r"$M_\chi$ [eV]", fontsize=24)
plt.ylabel(r"$1/\sigma_{g_\chi}$", fontsize=24)
plt.legend(fontsize=18, loc='lower left')
plt.tick_params(axis='x', which='minttor')
plt.grid(True, which='minor')
plt.savefig(ps23_resultsdir + "/plotA23b.png")
